...
function hasTag($item, tagName) {
    var tags = String($item.data("tag") || "")
        .split(",")
        .map(function (tag) { return tag.trim(); })
        .filter(Boolean);
    return $.inArray(String(tagName).trim(), tags) !== -1;
}
function getActiveCategories() {
    var i;
    var $activeNavItem = $(".nav-left ul li>div.active");
    var activeRel = $activeNavItem.data('rel');
    var categories = activeRel ? String(activeRel).split('<--->') : [];
...
function filterPostsByTag(tagName) {
    var i;
    var navState = getActiveCategories();
    var currentTag = String(tagName || '').trim();
    $('#search-panel').show().siblings().hide();
    $('#title-list-nav').show();
    $('#local-search-result').hide();
    $outlineList.hide();
    $('#default-panel > .right-title').text('#' + currentTag);
    $('#no-item-tips').hide();
    ...
    $(".nav-right nav a").each(function () {
        var $item = $(this);
        var inCategory = true;
        if (navState.activeRel) {
            for (i = 0; i < navState.categories.length; i++) {
                if (!$item.hasClass(navState.categories[i])) {
                    inCategory = false;
                    break;
                }
            }
        }
        $item.toggle(inCategory && hasTag($item, currentTag));
    });
    if ($(".nav-right nav a:visible").length === 0) {
        $('#no-item-tips').show().html('未找到标签为 <span>' + currentTag + '</span> 的文章');
    }
}
...
$(".nav-left ul li>div").on("click", function (e) {
    if ($(this).data('tag-search')) {
        $(".nav-left li>div.active").removeClass("active");
        $(this).addClass("active");
        $searchInput.val("#" + $(this).data('tag-search'));
        filterPostsByTag($(this).data('tag-search'));
        return;
    }
...
function inputChange() {
    ...
    var $activeNavItem = $(".nav-left ul li>div.active")
    var activeRel = $activeNavItem.data('rel')
    var activeTag = $activeNavItem.data('tag-search')
    var categories = activeRel ? String(activeRel).split('<--->') : [$('#default-panel > .right-title').text() || 'All']
    // 处理特殊字符
    for (i = 0; i < categories.length; i++) {
        categories[i] =  categories[i]
          .replace(/(?=\/|\\|#|\(|\)|\[|\]|\.)/g, "\\")
    }
    var activeTitle = activeRel ? categories.join('.') : __('all_articles')
    var searchType = '';
    var containType = '';
    $('#no-item-tips').hide()
    $(".nav-right nav a .post-title .search-keyword").each(function () {
    ...
    if (val === "") {
        ...
        searchType = '标签'
        containType = '为'
        $('#default-panel > .right-title').text(activeTag ? ('#' + activeTag) : val)
        if (val.substr(1).length !== 0) {
            filterPostsByTag(val.substr(1).trim());
        }
    } else if (val.substr(0, 1) === "@") {
        searchType = '作者'
        containType= '为'
        ...
        searchType = '标题'
        containType = '包含'
        $('#default-panel > .right-title').text(activeRel ? categories[categories.length - 1] : __('all_articles'))
        // $(".nav-right nav a").css("display", "none");
        if (activeRel) {
            ...
            $(".nav-right nav").find("a:"+ ($('#search-panel > .icon-case-sensitive').hasClass('active') ? 'containsSensitive' : 'contains') + "('" + val + "')").css("display", "block");
        }
        $(".nav-right nav a").each(function () {
...
function syncSingleOutline($list, _this) {
        if ($list.length && $list.is(':visible') && $list.find('.toc-link[href!="#"]').length > 0 && !clickScrollTo) {
            var activeIndex = null
            var $links = $list.find('.toc-link[href!="#"]')
            $links.each(function (index) {
                var $target = $(_this).find(decodeURI($(this).attr('href')))
                if (!$target.length) {
                    return true
                }
                var diff = _this.scrollTop - $target[0].offsetTop
                if (diff < -20) {
            ...
            if (activeIndex === null) {
                ...
                $links.eq(activeIndex).addClass('active')
            }
            var $activeLink = $links.filter('.active')
            if ($activeLink.length && $activeLink[0].offsetTop - $list.height() - $list[0].scrollTop > -80) {
                $list.scrollTop($activeLink[0].offsetTop + 80 - $list.height())
            } else if ($activeLink.length && $activeLink[0].offsetTop < $list[0].scrollTop) {
                $list.scrollTop($activeLink[0].offsetTop)
            }
        }
}
function syncOutline(_this) {
    try{
        syncSingleOutline($outlineList, _this)
        syncSingleOutline($('#article-toc-list'), _this)
    } catch (e) {
...
$(function () {
    ...
    /** tag - start **/
    // 切换 tag 面板
    $('#search-panel > .icon-tag, #default-panel > .icon-tag').on('click', function (e) {
        e.stopPropagation()
...
function bind() {
    ...
    // 修复自定义标题的关联关系
    $("#outline-list, #article-toc-list").find('.toc-link').each(function() {
        if (!$(this).attr('href')) {
    ...
    $("a[href^='#']").click(function () {
        var $this = $(this)
        if ($this.parents('#outline-list, #article-toc-list').length > 0) {
            $('#outline-list .toc-link[href!="#"].active, #article-toc-list .toc-link[href!="#"].active').removeClass('active')
            $this.addClass('active')
        }
        clickScrollTo = true
